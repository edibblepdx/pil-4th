...
-- exercise: modify to read from file containing a list of words to ignore

-- step 0: collect words to ignore
local ignore = {}
file = arg[2] and assert(io.open(arg[2]), "Not a valid file") or nil
...
for line in io.lines() do
    for word in string.gmatch(line, "%w+") do -- sequences of one or more alphanumeric characters
        if not ignore[word] then counter[word] = (counter[word] or 0) + 1 end
    end
end
